...
| [FIX-RADIO-LAYOUT-GAPS](FIX-RADIO-LAYOUT-GAPS/PRD.md) — three gaps in the preset-plan radio data, all surfaced by converting Foothold over a 32-type fleet: a radio-compass classified as an FM radio (so a 30-channel list lands on an ADF), the AJS-37's `trailing_specials` below its radio's floor, and **no Flaming Cliffs aircraft at all** in `dcs-radio-specs.yaml` | ⬜ |
