...
| Lot | Status |
|-----|--------|
| [FIX-MISSILEGUARDIAN-INIT-CRASH](FIX-MISSILEGUARDIAN-INIT-CRASH/PRD.md) — `MISSILEGUARDIAN: true` crashed VEAF start-up (`veafMissileGuardian.initialize()` called the never-defined `dumpMissionsList`), aborting `veaf-config.lua` and silently disabling the central F10 marker dispatcher (dead `_spawn`/shortcut aliases despite `SHORTCUTS: true`), plus CTLD/CSAR init. Reported by Tripack | ✅ |
