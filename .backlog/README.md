...
| [TOOLING-DOC-AUTOGEN](TOOLING-DOC-AUTOGEN/PRD.md) — `docs-check` validates that links resolve and anchors exist; it cannot tell whether a reference still describes the code. DOC-AUDIT-PASS found a stale `addSubMenu` signature and a "6.5.25 / June 2026" header in `LUA_API_REFERENCE` — silent rot — and this session added 73 more lines to that file **by hand**. dcs-sms regenerates its 141 CLI pages with one command and fails a `cli-docs-fresh` CI job when the committed result differs; that mechanism is the thing worth having. The scoping decision is which references are actually **derivable**, and getting it wrong means promising a generator for prose: `ALIASES.md` (8 KB) is a rendering of `veaf_libs/data/veaf-units.yaml`, and the MCP catalogue page is derivable from `list_catalog()` which already returns every action's full spec — generate both, and the second **retires a standing trap**, since adding an action currently means *remembering* to update the catalogue by hand and an action missing from it is invisible to the agent that needs it. `LUA_API_REFERENCE` (118 KB), `MISSION_YAML_REFERENCE` (39 KB) and `PIPELINE_REFERENCE` (31 KB) are **explicitly out of scope**: they are prose explaining behaviour, caveats and in-game consequences, so a generator would destroy the useful part to automate the trivial one — the PRD says so to stop the next person trying. The catalogue generator emits between markers so the hand-written framing survives, and both tickets require reading the diff against the current hand-maintained version line by line: anything the doc knows that the data does not is a **gap in the data**, to be fixed there rather than papered over in a template. The gate is the deliverable, not the generators — without it they are optional and the docs rot again, just more slowly — and it must carry no path filter that lets a change to `veaf-units.yaml` or the catalogue skip the job, which is exactly the case it exists for (FIX-WORKFLOWS-MAIN-TO-MASTER is what a never-triggering job costs) | ⬜ |
