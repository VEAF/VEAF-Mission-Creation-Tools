...
| Lot | Status |
|-----|--------|
| [FIX-SECREV2-EXPIRED-DEFERRALS](FIX-SECREV2-EXPIRED-DEFERRALS/PRD.md) — a deferral is only honest if something eventually collects it, and two of `SECREV-2`'s came due unnoticed. **VMR-088** was deferred *to* `REFACTOR-MARKER-PARSER`, which closed the same day **without touching it** — `veafCombatMission.lua` is not one of the parsers it migrated — so it now points at a lot that no longer exists; its real half is a correctness bug, `getRemainingEnemies` reading a unit's life **four times** per unit (the review said three; the fourth hides in a trace), so a unit under fire can be classified inconsistently and the remaining-enemies count a player trusts goes wrong. **VMR-013** keeps the fiddle-server port executing arbitrary Lua from unauthenticated HTTP, deferred because no DCS was available to test a token on the transport the smoke harness speaks through — the harness has since run in game, but its ticket 04 **keeps the hook**, so the dependency is live and nothing links the two; its triage entry also notes the review **understates** the severity, since `cors='*'` plus a GET channel means any web page visited while the hook is installed gets code execution. Found while restoring the triage that archiving `SECREV-2` had deleted **Ticket 01 done**: one read instead of four, and measuring gave numbers neither the review nor I had right — it was **2 calls on the alive path and 4 on the damaged one**, because each branch read again. The `else` branch whose comment claimed it could never be reached is reachable now that a single read decides, and counts nothing on purpose. Ticket 02 (the fiddle-server port) needs a DCS session | 🔄 |
| [FEAT-ASSIST-FOLLOWUP](FEAT-ASSIST-FOLLOWUP/PRD.md) — the three things `FEAT-ASSIST-CHECKLISTS` left open when it was archived, and which nothing was tracking: **resource names should carry a content hash** (DCS caches embedded resources by name, so a rebuilt checklist image keeps its stale bitmap until a full restart — the symptom, *"the text is wrong but only on the first image"*, points nowhere near the cause and cost an evening), **two pilots at once** (whether an `a_cockpit_highlight` leaks into another cockpit; the per-session id exists for it and has never been exercised), and **a pilot review of the F-16C slice** (the engine was flown and works; its six steps were never checked by an F-16C pilot). Ticket 01 is actionable now; 02 and 03 are blocked on cockpit time — and 02 on a second pilot | ⬜ |
...
| Lot | Status |
|-----|--------|| [BUILD-AUTOVERSION](archive/BUILD-AUTOVERSION.md) — auto-compute the release build number | ✅ || [BUILD-COMMUNITY-SOUNDS](archive/BUILD-COMMUNITY-SOUNDS.md) — Build owns CTLD/CSAR sound preloading | ✅ || [BUILD-PUBLISH-LOCAL](archive/BUILD-PUBLISH-LOCAL.md) — local publish mode for `veaf-build` | ✅ || [CHATBOT-CLI-WORKER](archive/CHATBOT-CLI-WORKER.md) — `ask` proxies the Worker (no user key) | ✅ |
