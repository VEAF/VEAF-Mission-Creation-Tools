...
### Added
- **In-game messages localized across the framework** (LUA-I18N-004, FR default + EN). Building on the i18n layer, the discrete pilot-facing messages of the VEAF modules are now routed through `veaf.t` with FR + EN catalog entries: spawn (unit/group/cargo/static/teleport/convoy/IADS), combat zones / combat missions / missile guardians (shared `entity.*` activation states), CAS and transport mission calls, ground/tanker/AFAC moves, radio, security (password/lockdown), skynet helper, named points, ground AI, weather fog, and assets status/help. Default output is now **French** unless `mission.language: en`. Large dynamically-assembled status/report builders (e.g. the weather/ATC report, carrier-ops and air-wave status, and a few long help/report blocks) are intentionally left for a follow-up pass — they mix data, coordinates and mission-configurable templates
