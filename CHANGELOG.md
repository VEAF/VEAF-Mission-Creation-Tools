...
### Added
- **`veaf-tools ask` — documentation chatbot in the CLI/TUI** (CHATBOT-CLI). Ask a question about the VEAF docs and get a grounded AI answer, reusing the **same RAG index** built by the docs CI. One-shot (`veaf-tools ask "how do I enable CTLD?"`) or an interactive REPL, plus a TUI entry « Ask the documentation ». The index (`vec-{lang}.bin` + `txt-{lang}.json`) is published as assets on a rolling `doc-index` GitHub Release; the tool downloads and caches it under `~/.veaf/doc-index/` (ETag-checked), then embeds the question and generates the answer with the **user's own** `GEMINI_API_KEY` (env var or `gemini_api_key` in `~/veafmct.yaml`) — no Cloudflare credentials, no local re-embedding. A Gemini 429 maps to a clear "too many requests" message; a missing key gives an actionable hint
