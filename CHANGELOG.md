...
### Fixed
- **The default `spawnables.yaml` / `dynamic-slot-templates.yaml` shipped a full demo roster** (~160 groups), which the build injected into any mission lacking its own files — adding dozens of late-activation client slots, so taking a slot showed the DCS *"YOUR FLIGHT IS DELAYED TO START, PLEASE WAIT"* message. Surfaced by AIRCRAFT-INJECT making `spawnables.yaml` actually injected. Both default files are now **empty** (`coalitions: {}`); populate them via `extract-aircraft-groups` or `convert-v5`. A regression test asserts the defaults carry zero groups (FIX-DEFAULTS-AIRCRAFT-ROSTER-001)
