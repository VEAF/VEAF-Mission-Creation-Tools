...
- **Adding, renaming or retitling a page? Refresh the support bot's index too**: `cd services/support-bot && poetry run python scripts/refresh_doc_pages.py`, and commit the regenerated `veaf_support_bot/doc_pages_data.py`. The bot links the pages its answers cite without carrying a copy of `doc/`, so that title index is checked in; its own `quality` job rebuilds it and fails on drift. **`docs-check` does not catch this** and neither does the root `pytest` — the service has its own suite, so a page added without the refresh is a red CI on the pull request and nothing before it.
...
- **Command examples run in PowerShell**, which is the default shell on Windows and the one mission makers get. Always write the executable as `.\veaf-tools.exe`, never `veaf-tools.exe`: PowerShell does **not** search the current directory (a deliberate protection against command hijacking), so the bare form fails with *"is not recognized as a name of a cmdlet, function, script file, or executable program"* — the single most confusing error a newcomer can meet, since the file is plainly right there. `cmd.exe` does search the current directory and accepts both forms, so **`.\` is the portable one** and the only one worth writing. Where a page teaches the command line rather than merely quoting a command, say that PowerShell is the default and that `cmd` differs, and explain how — `.\` optional there, `$env:VAR` vs `%VAR%`, backtick vs caret for line continuation. Measured 2026-09-01: bare `veaf-tools.exe` fails in PowerShell and works in `cmd`; `.\veaf-tools.exe` works in both.
