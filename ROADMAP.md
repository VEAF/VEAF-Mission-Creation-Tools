...
| **Dependencies** | **DROP-MIST** | Remove the hard dependency on MiST entirely — VEAF scripts no longer require or inject MiST. | MiST is injected unconditionally today. Foundation for the campaign work below. **David, 2026-08-17: he wants to try removing it outright, and there is now a precedent — CTLD 2 dropped it.** So the question is no longer *whether* a VEAF-scale script can live without MiST, but which of our calls have no native equivalent. Worth noting from the #290 investigation the same day: `mist.teleportToPoint` was read end to end and **does its job correctly** (it deep-copies the route, translates every waypoint by the teleport delta, and hands the group to `dynAdd` with its route attached) — so this is not a lot born of distrust in MiST. The reasons stand elsewhere: an unconditional 500 KB dependency, a second scheduler, and every VEAF spawn path routed through a library nobody here maintains. Start by counting the call sites per MiST function, since that number decides whether this is a lot or a campaign. |
