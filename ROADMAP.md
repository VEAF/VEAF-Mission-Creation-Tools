...
| [`CHORE-VENDORED-DRIFT-618`](.backlog/CHORE-VENDORED-DRIFT-618/PRD.md) | 🔄 | Artefacts synced (CTLD rc8, TUM 0.3) and released in 6.22.0. **Ticket 02 waits on David**: the drift watch asks GitHub for `/releases/latest`, which skips pre-releases — and every VEAF/CTLD release is one, so rc8 went unreported for seventeen days across three weekly runs. Four options in the ticket; changing it contradicts a note written in `vendored.yaml`, which is why it is not a keyboard decision |
...
| **Missions** | **MISSION-RECIPES** | Des « recettes » déclaratives qui décrivent la suite d'opérations outillées produisant un type de mission, lancées par `veaf-tools` : une mission de démo toujours fraîche où chaque nouveauté dépose son exemple, des missions de base (un théâtre, quelques bases par camp, des défenses, des QRA, des combat zones), des missions d'entraînement — le tout dans un catalogue. | 💡 **Idée de David, 2026-08-31**, à reprendre **après une release**. Analyse dans [`FEAT-MISSION-RECIPES`](.backlog/FEAT-MISSION-RECIPES/PRD.md). Le moteur existe déjà : `run_action(name, params)` du catalogue MCP ne fait intervenir aucun modèle — le LLM choisit les appels, il ne les exécute pas — donc il manque un format de fichier et un dérouleur, pas un moteur d'édition. **Le cas le plus fort est la démo, et pas pour sa fraîcheur** : régénérée en CI avec un exemple par fonctionnalité, elle devient le test d'intégration de bout en bout que le produit n'a pas. **Risque nommé d'avance** : « quelques bases rouges et bleues » suppose de décider *où*, et dériver les positions fait pousser variables, conditions et boucles — un projet en soi. D'où la reco : une seule recette bête d'abord, extraire un format au troisième. Quatre questions à trancher avant de cadrer (sortie dossier ou `.miz`, ce que la recette apporte que le squelette ne fait pas, idempotence, couplage au théâtre). |
