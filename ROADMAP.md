...
| **Quality** | **LUACHECK-CI** | Add `luacheck` to the CI Lua quality gate (only `stylua --check` runs today). | Closes the static-analysis blind spot on the Lua side — matters before DROP-MIST / PERSISTENCE start editing the runtime. |
| **Quality** | **LUA-COVERAGE** | Establish a Lua test-coverage objective for the runtime modules (`veafCasMission`, `veafCombatZone`, `veafQraManager`, …). | The Lua side is far less tested than Python; secures the campaign/persistence work that will touch these modules. |
| **Tooling** | **VALIDATE** | `veaf-tools validate`: lint `mission.yaml` + `.miz` **before** build — incoherent modules, missing `custom_scripts` files, presets/waypoints matching no aircraft, missing REDFOR/BLUFOR zones when TUM is on. | Turns late DCS-side crashes into clear design-time errors. Overlaps the active `FIX-CONVERT-V5-INVALID-YAML` lot. |
| **Tooling** | **SCAFFOLD** | `veaf-tools new`: scaffold a ready-to-use mission folder (typed `mission.yaml` + `src/` tree) from one or more templates (blank training, CAS, QRA…). | Lowers the entry cost for new mission makers. |
