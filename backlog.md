...
| DCSDATA-005 | Per-PR consistency guard: CI re-runs the **countries** generator against the pinned ref and `git diff --exit-code`, failing when the committed `dcs-countries.yaml` is stale or hand-edited. **Scope note**: only pure artifacts (countries) can use a regenerate-equality guard — radio has manual overlays, so it is excluded. | `.github/workflows/` | chore | ⬜ |
