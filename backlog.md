...
| Lot | Status |
|-----|--------|
| Lot FEAT-MIGRATE-MISSION-V6 — `convert-v5` migrates a mission folder's Lua/config to v6 but leaves `src/mission/` (the exploded `.miz`) in v5; the build re-applies the v5→v6 trigger migration **in memory on every build** (`migrate_from_v5=True`). Add a command that **promotes `src/mission/` to v6 on disk** via a build→extract round-trip (build in memory → extract back into `src/mission/`, with a `backup_v5/src/mission/` safety copy), giving the mission-maker a definitive v6 switch and paving the way to drop `migrate_from_v5` from the build. Fidelity is **functional, not binary** (the build injects data). Pre-req: confirm every injector is idempotent on rebuild when `src/mission/` already holds its output (aircrafts ✅ + spawn-data ✅ verified; waypoints/presets/warehouses/weather to audit) (David) | ⬜ |
...
**Goal**: a command (name TBD, e.g. `veaf-tools migrate-mission`) that promotes `src/mission/` to v6 on disk via a **build→extract round-trip**: build the mission in memory (full v6 injection, v5 triggers migrated), then extract the result back into `src/mission/`, after copying the current `src/mission/` to `backup_v5/src/mission/`. Two payoffs: (1) a **definitive v6 switch** for the mission-maker (the source becomes a clean v6 mission, editable/buildable with no v5 legacy), and (2) it makes `migrate_from_v5` **redundant** for migrated missions, paving the way to remove that build-time debt. **Functional (not binary) fidelity** is expected and accepted — the build injects data, so the exploded v6 differs from the exploded v5 by construction.

**Pre-req / risk**: the round-trip is only safe if **every** injector is idempotent on rebuild when `src/mission/` already holds its output (no duplicates). Verified: aircrafts (skip/replace by name, explicitly for the post-conversion case) and spawn-data (Lua resource stripped on extract + VEAF trigger remove/reinject). **To audit**: waypoints, presets, warehouses, weather.

**Branch**: `feature/migrate-mission-v6` → PR → `develop-v6`.

| # | Ticket | Files | Type | Status |
|---|--------|-------|------|--------|
| FEAT-MIGRATE-MISSION-V6-001 | **Idempotence audit (spike)**: confirm the waypoints/presets/warehouses/weather injectors never duplicate when `src/mission/` already holds their output (the rebuild-after-promote scenario). Document the per-injector idempotence contract; add a regression test that builds → extracts → rebuilds and asserts stability (group/trigger counts unchanged). | `*_injector/`, `test/python/` | spike | ⬜ |
| FEAT-MIGRATE-MISSION-V6-002 | **`migrate-mission` command**: build in memory (full v6 injection) → extract back into `src/mission/`, after copying the current `src/mission/` to `backup_v5/src/mission/`. Emit a short report (what changed, backup location). Add to the TUI (`CommandSpec`, per FIX-TUI-MISSING-COMMANDS guard). TDD. | `veaf_tools/commands/`, `mission_builder/`, `mission_extractor/`, `veaf_libs/tui.py`, `test/python/` | feat | ⬜ |
| FEAT-MIGRATE-MISSION-V6-003 | **Deprecate `migrate_from_v5`**: once a mission can be promoted on disk, the in-memory build migration is redundant for migrated missions. Emit a build warning when `migrate_from_v5` actually migrates something (source still v5) inviting the maker to run `migrate-mission`; keep the flag for back-compat. Plan the code removal once the ecosystem is migrated. | `mission_builder/mission_builder_worker.py`, `veaf_tools/commands/build.py`, `test/python/` | chore | ⬜ |
