...
| DCSDATA-007b | Lift the 2-ground-group requirement by having the build inject a minimal **real** placeholder ground unit into any *used* coalition that has none (a unit-less country gets purged by DCS — see 007). Open design points to settle (validated with a preview `.miz` before wiring): placeholder shape (1 infantry soldier vs static), `hidden`/`lateActivation` flags vs runtime coalition registration (cf. RC-018 / MIST `DBs.units`), country choice (e.g. CJTF Blue id 80 / CJTF Red id 81), pipeline location. Update `doc/mission-maker/GUIDE.*.md`. | `mission_builder/`, `doc/mission-maker/GUIDE.*.md`, `test/python/` | feat | ⬜ |
| DCSDATA-008 | Move the DCS **units** database onto the datamine, retiring the in-DCS export (`dcsDataExport.lua`). Investigation confirmed the datamine carries every field veafUnits actually consumes (`type`, `description`, `category`, `naval`/`air`/`infantry`/`vehicle` booleans derivable from category+attributes, `attribute` flags, cargo `maxMass`/`minMass`); the only apparent blocker (`desc.box` geometry) is dead/commented code absent from the committed file. Build a `units` provider that re-emits `dcsUnits.lua` in its exact runtime Lua schema (`DcsUnitsDatabase`, `NavalStatics` via `isPutToWater`, cargo `desc`), **gated by a semantic-equivalence diff** against the committed file (keep the export for any field the datamine lacks). Wire `--units` into `update-dcs-data` (now a pure, guard-eligible artifact) and point `dcs_units_parser` at the generated source. | `veaf_build/dcs_data/`, `src/scripts/veaf/dcsUnits.lua`, `veaf_libs/dcs_units_parser.py`, `test/python/` | feat | ⬜ |
