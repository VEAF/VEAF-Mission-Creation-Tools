...
| FOOTHOLD-V6-007 | Caucasus pilot + process doc — integrate Caucasus end-to-end (mission folder committed in `VEAF-Foothold-Caucasus`), document the moulinette (init + `--update`) for a third party, validate iso-functionally in DCS. 🧑 **Gate (David)**: manual DCS test of both variants. **Moulinette doc done** (`doc/mission-maker/FOOTHOLD.{md,en.md}`) and the procedure was exercised end-to-end on the real Caucasus `.miz` (one `build` → MODERN + COLD_WAR `.miz`, each with the right `Era`). **Remaining (David)**: commit the v6 mission folder into `VEAF-Foothold-Caucasus` (currently v5-structured) and the iso-functional DCS test. | `VEAF-Foothold-Caucasus` repo, `doc/` | feat | 🧑 |
