...
| FOOTHOLD-V6-008 | Honour `custom_scripts:` declaration order in the mission-script load sequence — today the load order follows glob/collection order, not the declared order, so the ordered `custom_scripts:` block scaffolded by 001 is decorative for sequencing. Reorder the collected mission scripts by their `custom_scripts.scripts[].path` declaration order (unlisted files keep their current relative order, appended). Generic (also benefits non-Foothold VEAF missions). Surfaced by FOOTHOLD-V6-004. | `mission_builder/mission_builder_worker.py`, `test/python/` | feat | ⬜ |
