...
**Investigation (Yak-52 repro analysed — `freq-yak52.miz`)**: the ticket premise is likely **inverted**, and the failing stage is **not** a scale conversion. Established facts:
- **DCS stores the ARK-15M in MHz**, at unit level: `unit.Radio[1].channels = { 0.625, 0.303, … }`. The Mission Editor *displays* kHz (625) but *stores* MHz (0.625). So `0.625` is the **correct native value**; writing `625` into the `.miz` would be 625 MHz, far outside the ARK range (0.1–1.795) → DCS would reject it. (So Tripack's "0.625 → 625 fixes it" needs in-game re-confirmation: does 625 actually *load and tune the right station*, or just stop crashing for another reason?)
...
**Leading hypothesis (H1)**: the bug is not a ×1000 scaling — it's that excluding `"Radio"` **strips the ARK-15M channels** from an injected Yak-52 dynamic-slot template, so the spawned slot loses its frequencies. Fix would be to preserve `Radio.channels` for aircraft with custom ADF channels rather than blanket-excluding `Radio`. **Needs from Tripack**: (1) which exact file was edited `0.625 → 625` (the `dynamic-slot-templates.yaml`, or the built `.miz`?); (2) in-game confirmation that `625` both loads **and** tunes correctly; (3) whether the crash is at mission load or at dynamic-slot spawn. **Status: needs Tripack clarification before coding (premise contradicted by DCS storage).**
