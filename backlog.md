...
**Second symptom — same root cause (Tripack)**: a QRA only triggers on an airplane dynamic-slot when `react_on_helicopters: true`; with it `false`/absent the QRA ignores the airplane. Confirmed in `veafQraCore.lua`: the intruder filter keys on **group category** — `(unitCategory == AIRPLANE) or (HELICOPTER and reactOnHelicopters)` (line 632) and `(unit.category == "plane") or ("helicopter" and reactOnHelicopters)` (line 666), and `group:getCategory()` (line 774) — which reflects the `helicopters:`/`airplanes:` section the group is filed under in the `.miz`. So an airplane filed under `helicopters:` is seen as a helicopter and only triggers a QRA when `reactOnHelicopters` is set. **Hypothesis**: fixing 001 (airplanes filed under `airplanes:`) resolves this too. Verify after 001 (ticket 002); only needs independent QRA work if it persists.
...
| FIX-DYNSLOT-TEMPLATE-CATEGORY-002 | After 001, verify the QRA second symptom is gone: an airplane dynamic-slot must trigger a QRA with `react_on_helicopters: false`/absent. Repro: dynamic-slot airplane + a QRA whose `react_on_helicopters` is false → the QRA must activate. If it still fails after 001, investigate QRA intruder-category detection for dynamic slots independently. Depends on 001. | `veafQraCore.lua` (only if it persists), DCS manual test (Tripack) | fix | ⬜ |
