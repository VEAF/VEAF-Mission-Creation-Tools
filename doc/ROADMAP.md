...
- 🔵 **`missionconfig.lua`** — mission-level file that overrides module defaults; loaded at startup before init
- 🔵 **Deferred module initialization** — load order: `require` → `missionconfig` → `veaf.initialize()` (backward compatible)
- 🔵 **`generate-config` command** — generates a documented `missionconfig.lua` template for a given mission
- 🔵 **Mission YAML → module selection** — `lua_modules` section in `mission.yaml` drives which modules are included
...
- ✅ Lua CI on GitHub Actions (`lua-unit-tests` + `stylua-check`)
- 🔵 Python unit tests — pytest with coverage (target ≥ 60%)
