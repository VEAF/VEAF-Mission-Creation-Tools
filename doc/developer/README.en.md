...
|-----------------|----------------|
| Lua files (`src/scripts/veaf/`, `test/lua/`) | `stylua --check src/scripts/veaf/ test/lua/` then `poetry run test-lua` |
| Python code (`src/python/`, `test/python/`, `veaf_build/`) | `poetry run ruff check src/python/ test/python/ veaf_build/` then `poetry run ruff format --check src/python/ test/python/ veaf_build/` then `poetry run mypy src/python/veaf-tools` then `poetry run pytest` |
...
- [Testing Guide](../TESTING.en.md) — **Lua** test infrastructure in detail; the Python side (pytest, shared helpers) is covered in the [developer guide](GUIDE.en.md#shared-test-helpers)
...
- **A repo that publishes nothing but pre-releases**: GitHub's `/releases/latest` skips them by design, so such a repo answers 404 and reads as an error for ever — CTLD sat 17 days behind that way. Set `prereleases: true` on the watch and the releases are listed instead. Do **not** set it on a repo that cuts stable releases: a beta would then read as drift. Add `tag_pattern` (a regular expression, e.g. `^published-v`) wherever the repo also republishes a moving tag — CTLD rewrites `dev` on every `master` build, and without a pattern that tag is always the newest thing there.
