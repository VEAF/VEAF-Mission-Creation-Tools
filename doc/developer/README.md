...
|--------------------|-------------------|
| Fichiers Lua (`src/scripts/veaf/`, `test/lua/`) | `stylua --check src/scripts/veaf/ test/lua/` puis `poetry run test-lua` |
| Code Python (`src/python/`, `test/python/`, `veaf_build/`) | `poetry run ruff check src/python/ test/python/ veaf_build/` puis `poetry run ruff format --check src/python/ test/python/ veaf_build/` puis `poetry run mypy src/python/veaf-tools` puis `poetry run pytest` |
...
- [Guide de test](../TESTING.md) — infrastructure de test **Lua** en détail ; le côté Python (pytest, aides partagées) est couvert dans le [guide développeur](GUIDE.md#shared-test-helpers)
...
- **Dépôt qui ne publie que des pré-versions** : l'API GitHub `/releases/latest` les saute par construction, donc un tel dépôt répond 404 et se voit classé en erreur indéfiniment — CTLD est resté 17 jours en retard de cette façon. Poser `prereleases: true` sur la surveillance fait lister les releases au lieu d'interroger `/releases/latest`. À ne **pas** poser sur un dépôt qui publie des versions stables : une bêta y passerait pour une dérive. Ajouter `tag_pattern` (expression régulière, ex. `^published-v`) partout où le dépôt republie aussi une étiquette mobile — CTLD réécrit `dev` à chaque construction de `master`, et sans motif c'est toujours elle la plus récente.
