...
| `build` | Construit la mission depuis `src/` — injecte les triggers VEAF, produit un `.miz`. Valide au passage toutes les références de `mission.yaml` vers le Mission Editor (zones de déclenchement, groupes, unités, aérodromes) : il signale les avertissements, collecte toutes les erreurs, va au bout, puis **échoue** s'il en reste (réf. obligatoire absente). Une zone AIRWAVES avec `trigger_zone_name` absente mais avec centre/rayon configurés ne donne qu'un avertissement |
