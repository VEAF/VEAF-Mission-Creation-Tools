...
#: Default location of missionConfig.lua relative to the mission folder.
...
class V5Converter:
    ...
    def _migrate_config(self, report: ConversionReport, backup: bool) -> None:
        """Run ConfigMigrator on missionConfig.lua and write the result in-place."""
        ...
        # Write migrated content in-place
        src.write_text(result.new_content, encoding="utf-8")
        report.missionconfig_output = src
    ...
    def _build_mission_yaml(self, report: ConversionReport) -> str:
        ...
        lines: list[str] = [
            ...
            "# Modules not listed here use the builder default (all enabled).",
            "#",
            "lua_modules:",
        ]

        enabled_modules = report.migration_result.enabled_modules if report.migration_result else []
        enabled_set = set(enabled_modules)
        all_mods = get_modules()

        enabled_found = [m["id"] for m in all_mods if m["id"] in enabled_set]
        disabled_found = [m["id"] for m in all_mods if m["id"] not in enabled_set]
