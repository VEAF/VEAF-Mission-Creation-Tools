...
def generate_config_lua(
    ...
    header:
        Comment text prepended after the separator line.
    ...
    if lua_modules:
        ...
        for mod_id in ordered_ids:
            ...
            if not var_name:
                lines.append(f"-- NOTE: no Lua variable found for module {mod_id!r} — skipped")
                lines.append("")
                continue
...
def generate_mission_yaml_template(
    ...
    lines: list[str] = [
        "# mission.yaml — VEAF mission configuration",
        "# Generated by: veaf-tools generate-config",
        "#",
        "# Place this file at the root of your mission folder.",
        "# All sections are optional — remove or comment out what you don't need.",
        "# See: https://github.com/VEAF/VEAF-Mission-Creation-Tools/blob/master/doc/MISSION_MAKER_GUIDE.md",
        "",
        "# ── Global log level ─────────────────────────────────────────────────────────",
        "# Forces all VEAF loggers to this level. Remove or set to 'info' for players.",
        ...
        "",
        "# ── Generic settings ─────────────────────────────────────────────────────────",
        "# Arbitrary veaf.config.XXX = value assignments.",
        "# settings:",
        '#   MY_SETTING: "value"',
        "",
        "# ── Module configuration ─────────────────────────────────────────────────────",
        "# Enable/disable individual VEAF Lua modules and override their log level.",
        "# Modules not listed here default to enabled.",
        "#",
        "lua_modules:",
    ]
    ...
    if enabled_found:
        lines.append("  # ── Active modules (found in missionConfig.lua) ─────────────────────────")
    ...
    if disabled_found or remaining:
        lines.append("  # ── Other modules (not found / disabled by default) ──────────────────────")
    ...
    lines += [
        ...
        "# Requires QRA module enabled above. Each entry creates a VeafQRA builder chain.",
        "# qra:",
        "#   silence_all: false",
        "#   definitions:",
        '#     - name: "Base QRA"',
        ...
        "",
        "# ── CAP missions ──────────────────────────────────────────────────────────────",
        "# Requires COMBATMISSION module enabled above.",
        "# cap_missions:",
        ...
        "# Each step auto-detects its config file; set to false to skip.",
        "#",
        "# pipeline:",
