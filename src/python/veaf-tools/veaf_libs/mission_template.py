...
def generate_mission_yaml(enabled: set[str]) -> str:
    ...
    The file carries the same rich, tier-independent preamble as ``generate-config``
    and ``convert-v5`` (YAML syntax guide, ``global_log_level:``, ``mission:`` identity,
    ``security:``) plus a trailing ``pipeline:`` section — sourced from the shared
    helpers in :mod:`veaf_libs.lua_config_generator` so the scaffolds stay in lockstep.
    Only the ``modules:`` block is tier-driven and specific to ``prepare``.

    Args:
        enabled: Module ids to include (infrastructure is always included regardless).

    Returns:
        The full ``mission.yaml`` content (header + preamble + ``modules:`` + ``pipeline:``).
    """
    from veaf_libs.lua_config_generator import (
        global_log_level_section,
        mission_identity_section,
        pipeline_section,
        security_section,
        yaml_syntax_header,
    )
    ...
    lines.append("")
    lines.extend(mission_identity_section("My-Mission"))
    lines.append("")
