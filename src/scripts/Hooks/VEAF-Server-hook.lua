...
function veafServerHook.p(o, level)
  local MAX_LEVEL = 20
  if level == nil then
    level = 0
  end
  ...
  if type(o) == "table" then
    ...
    for key, value in pairs(o) do
      ...
      text = text .. "." .. key .. "=" .. veafServerHook.p(value, level + 1) .. "\n"
    end
...
function veafServerHook.onSimulationStart()
  ...
  local _status, _retValue = pcall(
    ...
    "mission",
    "return a_do_script("
      .. "[===[ if veaf and veaf.getMissionMaxRuntime then return veaf.getMissionMaxRuntime() else return nil end ]===]"
      .. ")"
  )
  ...
  if veafServerHook.maxMissionDuration == nil then
    veafServerHook.maxMissionDuration = veafServerHook.DEFAULT_MAX_MISSION_DURATION
    veafServerHook.logInfo(
      string.format("Maximum mission duration is set to its default value (%s)", veafServerHook.p(veafServerHook.maxMissionDuration))
    )
  else
  ...
  veafServerHook.logInfo(
    string.format(
      "Maximum number of players for restart is set to its default value (%s)",
      veafServerHook.p(veafServerHook.maxPlayersForRestart)
    )
  )
...
function veafServerHook.onPlayerConnect(id)
  ...
  if not pilot then
    veafServerHook.logInfo(
      string.format("Unknown pilot [%s] connecting with UCID [%s]", veafServerHook.p(playerName), veafServerHook.p(ucid))
    )
...
function veafServerHook.onPlayerTrySendChat(playerID, message, all)
  veafServerHook.logDebug(
    string.format("veafServerHook.onPlayerTrySendChat([%s], [%s])", veafServerHook.p(playerID), veafServerHook.p(message))
  )
...
function veafServerHook.onSimulationFrame()
  ...
  if
    veafServerHook.enableAutoRestart
    and _now > veafServerHook.lastServerUptimeCheckFrameTime + veafServerHook.FRAME_CHECK_FREQUENCY_IN_SECONDS
  then
  ...
  if
    veafServerHook.enableBufferingSocket
    and veafServerHook.config
    and veafServerHook.config.activate
    and _now > veafServerHook.lastUDPFrameTime + veafServerHook.config.refreshDelay
  then
...
function veafServerHook.parse(pilot, playerName, ucid, unitName, message)
  ...
  if not pilot then
    veafServerHook.logWarning(
      string.format("Unknown pilot [%s] sent chat message [%s])", veafServerHook.p(playerName), veafServerHook.p(message))
    )
  ...
  elseif _module and _module:lower() == "restart" then
    ...
    if veafServerHook.enableAutoRestart and pilot.level >= 10 then
      ...
      local _message =
        string.format("[%s] is asking for mission restart when the last pilot disconnects from the server", veafServerHook.p(playerName))
  ...
  elseif _module and _module:lower() == "halt" then
    ...
    if veafServerHook.enableAutoRestart and pilot.level >= 10 then
      ...
      veafServerHook.closeServerAtLastDisconnect = true
      local _message =
        string.format("[%s] is asking for server halt when the last pilot disconnects from the server", veafServerHook.p(playerName))
      veafServerHook.logInfo(_message)
      veafServerHook.sendMessage(_message, 10)
      veafServerHook.stopMissionIfNeeded()
      return true
  ...
  elseif _module and _module:lower() == "pause" then
    ...
    if pilot.level >= 1 then
      local payload =
        string.format(RUN_COMMAND, tostring(playerName), tostring(pilot.level), tostring(unitName), tostring(_module), tostring(_command))
      veafServerHook.logTrace(string.format("payload=%s", veafServerHook.p(payload)))
...
function veafServerHook.loadPilots()
  ...
  if not file then
    veafServerHook.logError(
      string.format(
        "Cannot load pilots list file [%s]: %s -- no pilot will be recognized and every command will be denied",
        veafServerHook.p(filepath),
        veafServerHook.p(err)
      )
    )
    return
  end
...
function veafServerHook.setupBufferingSocket()
  ...
  package.path = package.path
    ...
    .. ";"
    .. lfs.writedir()
    .. "/Mods/services/BufferingSocket/lua/?.lua"
  package.cpath = package.cpath
    ...
    .. ";"
    .. lfs.writedir()
    .. "/Mods/services/BufferingSocket/bin/"
    .. "?.dll;"
  ...
  if veafServerHook.config.activate then
    veafServerHook.logDebug(
      string.format(
        "set up the socket to call the web server; host=%s and port=%s",
        veafServerHook.p(veafServerHook.config.host),
        veafServerHook.p(veafServerHook.config.port)
      )
    )
