...
function csar.addCsar(_coalition, _country, _point, _typeName, _unitName, _playerName, _freq, noMessage, _description)
  ...
  csar.woundedGroups[_spawnedGroup:getName()] = {
    side = _spawnedGroup:getCoalition(),
    originalUnit = _unitName,
    desc = _text,
    typename = _typeName,
    frequency = _freq,
    player = _playerName,
...
function csar.eventHandler:onEvent(_event)
  local status, err = pcall(function(_event)
    ...
    elseif _event.id == 15 then --player entered unit
      ...
      if
        _event.initiator.getName
        and _event.initiator:getName()
        and _event.initiator.getPlayerName
        and _event.initiator:getPlayerName()
      then
...
function csar.handleEjectOrCrash(_unit, _crashed)
  ...
  if csar.csarMode == 1 then
    ...
    if _unit:getPlayerName() ~= nil and csar.currentlyDisabled[_unit:getName()] == nil then
      ...
      csar.currentlyDisabled[_unit:getName()] = {
        timeout = (csar.disableTimeoutTime * 60) + timer.getTime(),
        desc = "",
        noPilot = _crashed,
        unitId = _unit:getID(),
        name = _unit:getName(),
  ...
  elseif csar.csarMode == 2 then -- disable aircraft for pilot
    ...
    if _unit:getPlayerName() ~= nil and csar.pilotDisabled[_unit:getPlayerName() .. "_" .. _unit:getName()] == nil then
      ...
      csar.pilotDisabled[_unit:getPlayerName() .. "_" .. _unit:getName()] = {
        timeout = (csar.disableTimeoutTime * 60) + timer.getTime(),
        desc = "",
        noPilot = true,
        unitId = _unit:getID(),
        player = _unit:getPlayerName(),
        name = _unit:getName(),
      }
  ...
  if _unit ~= nil and _unit:getPlayerName() ~= nil and _playerName == _unit:getPlayerName() then
        ...
        if _details.noPilot then
          if csar.disableAircraftTimeout then
            local _text = string.format(
              "This aircraft cannot be flow as the pilot was killed in a crash. Reinforcements in %.2dM,%.2dS\n\nIt will be DESTROYED on takeoff!",
              (_time / 60),
              _time % 60
            )
          ...
          if csar.disableAircraftTimeout then
            ...
            csar.displayMessageToSAR(
              _unit,
              _details.desc .. " needs to be rescued before this aircraft can be flown again!\n\nIt will be DESTROYED on takeoff!",
              10,
              true
            )
          end
    ...
    elseif csar.csarMode == 3 then -- No Disable - Just reduce player lives
      ...
      if _lives == nil or _lives > 1 then
        ...
        local _text = string.format(
          "You have run out of LIVES! Lives will be reset on mission restart or when your pilot is rescued.\n\nThis aircraft will be DESTROYED on takeoff!"
        )
...
function csar.initSARForPilot(_downedGroup, _freq)
  ...
  local _text = string.format("%s requests SAR at %s, beacon at %.2f KHz", _leader:getName(), _coordinatesText, _freq / 1000)
...
function csar.pickupUnit(_heliUnit, _pilotName, _woundedGroup, _woundedGroupName)
  ...
  if _unitsInHelicopter + 1 > _maxUnits then
    csar.displayMessageToSAR(
      _heliUnit,
      string.format("%s, %s. We're already crammed with %d guys! Sorry!", _pilotName, _heliName, _unitsInHelicopter, _unitsInHelicopter),
      10
    )
    return true
  end
...
function csar.orderGroupToMoveToPoint(_leader, _destination)
  ...
  end, { _group:getName(), _mission }, timer.getTime() + 2)
end
...
function csar.checkCloseWoundedGroup(_distance, _heliUnit, _heliName, _woundedGroup, _woundedGroupName)
  ...
  if csar.heliVisibleMessage[_lookupKeyHeli] == nil then
    if csar.autosmoke == true then
      csar.displayMessageToSAR(
        _heliUnit,
        string.format("%s: %s. I hear you! Damn that thing is loud! Land or hover by the smoke.", _heliName, _pilotName),
        30
      )
    else
      csar.displayMessageToSAR(
        _heliUnit,
        string.format("%s: %s. I hear you! Damn that thing is loud! Request a Flare or Smoke if you need", _heliName, _pilotName),
        30
      )
    end
  ...
  if _distance < 500 then
    if csar.heliCloseMessage[_lookupKeyHeli] == nil then
      if csar.autosmoke == true then
        csar.displayMessageToSAR(
          _heliUnit,
          string.format("%s: %s. You're close now! Land or hover at the smoke.", _heliName, _pilotName),
          10
        )
      else
        csar.displayMessageToSAR(
          _heliUnit,
          string.format("%s: %s. You're close now! Land in a safe place, i will go there ", _heliName, _pilotName),
          10
        )
      end
      --mark as shown for THIS heli and THIS group
    ...
    for _currentHeli, _groups in pairs(csar.inTransitGroups) do
      if _groups[_woundedGroupName] then
        ...
        if _group.side == _heliUnit:getCoalition() then
          ...
          csar.displayToAllSAR(
            string.format("%s has been picked up by %s", _woundedGroupName, _currentHeli),
            _heliUnit:getCoalition(),
            _heliName
          )
...
function csar.signalFlare(_unitName)
  ...
  if _closet ~= nil and _closet.pilot ~= nil and _closet.distance < 8000.0 then
    ...
    local _msg = string.format(
      "%s - %.2f KHz ADF - %.3fM - Popping Signal Flare at your %s ",
      _closet.groupInfo.desc,
      _closet.groupInfo.frequency / 1000,
      _closet.distance,
      _clockDir
    )
    csar.displayMessageToSAR(_heli, _msg, 20)
...
function csar.reqsmoke(_unitName)
  ...
  if _closet ~= nil and _closet.pilot ~= nil and _closet.distance < 8000.0 then
    ...
    local _msg = string.format(
      "%s - %.2f KHz ADF - %.3fM - Popping Blue smoke at your %s ",
      _closet.groupInfo.desc,
      _closet.groupInfo.frequency / 1000,
      _closet.distance,
      _clockDir
...
function csar.addMedevacMenuItem()
  ...
  for key, unitName in pairs(csar.csarFixedUnits) do
    if csar.csarUnits[unitName] == nil then
      ...
      for _woundedName, _groupInfo in pairs(csar.woundedGroups) do
        if
          false --[[ _group is nil here, can't ever work (see my comment https://github.com/ciribob/DCS-CSAR/commit/907524d1e97930061d5f32832523bc4f7455192d#commitcomment-126043845) _groupInfo.side == _group:getCoalition() ]]
        then
          -- Schedule timer to check when to pop smoke
    ...
    if _unit ~= nil then
      ...
      if _groupId then
        if csar.addedTo[tostring(_groupId)] == nil then
          ...
          missionCommands.addCommandForGroup(_groupId, "List Active CSAR", _rootPath, csar.displayActiveSAR, _unitName)
