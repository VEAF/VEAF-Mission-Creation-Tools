...
function Hercules_Cargo.Soldier_SpawnGroup(
...
function Hercules_Cargo.Cargo_SpawnObjects(
  Cargo_Drop_initiator,
  Cargo_Drop_Direction,
  Cargo_Content_position,
  Cargo_Type_name,
  Cargo_over_water,
  Container_Enclosed,
  ParatrooperGroupSpawn,
  offload_cargo,
  all_cargo_survive_to_the_ground,
  all_cargo_gets_destroyed,
  destroy_cargo_dropped_without_parachute,
  Cargo_Country
)
  ...
  if offload_cargo == true then
    ...
    if CargoHeading >= 3.14 then
      CargoHeading = 0
      Cargo_Drop_Position = {
        ["x"] = Cargo_Content_position.x - (30.0 * math.cos(Cargo_Drop_Direction - 1.0)),
      ...
      if CargoHeading >= 1.57 then
        CargoHeading = 3.14
        Cargo_Drop_Position = {
          ["x"] = Cargo_Content_position.x - (20.0 * math.cos(Cargo_Drop_Direction + 0.5)),
        ...
        if CargoHeading >= 0 then
          CargoHeading = 1.57
          Cargo_Drop_Position = {
            ["x"] = Cargo_Content_position.x - (10.0 * math.cos(Cargo_Drop_Direction + 1.5)),
    ...
    CargoHeading = 0
    Cargo_Drop_Position = {
      ["x"] = Cargo_Content_position.x - (20.0 * math.cos(Cargo_Drop_Direction)),
    ...
    if all_cargo_gets_destroyed == true or Cargo_over_water == true then
      if Container_Enclosed == true then
        ...
        if ParatrooperGroupSpawn == false then
          Hercules_Cargo.Cargo_SpawnStatic(
            Cargo_Drop_initiator,
            Cargo_Drop_Position,
            "Hercules_Container_Parachute_Static",
            CargoHeading,
            true,
            Cargo_Country
          )
      ...
      if all_cargo_survive_to_the_ground == true then
        ...
        if Container_Enclosed == true then
          if ParatrooperGroupSpawn == false then
            Hercules_Cargo.Cargo_SpawnStatic(
              Cargo_Drop_initiator,
              { ["z"] = Cargo_Drop_Position.z + 10.0, ["x"] = Cargo_Drop_Position.x + 10.0 },
              "Hercules_Container_Parachute_Static",
              CargoHeading,
              false,
              Cargo_Country
  ...
  local status, result = pcall(function()
    ...
    if next(Arg[1].Cargo_Contents) ~= nil then
      if Hercules_Cargo.Calculate_Object_Height_AGL(Arg[1].Cargo_Contents) < 5.0 then --pallet less than 5m above ground before spawning
        ...
        Hercules_Cargo.Cargo_SpawnObjects(
          Cargo_Drop_initiator,
          Arg[1].Cargo_Drop_Direction,
          Object.getPoint(Arg[1].Cargo_Contents),
          Arg[1].Cargo_Type_name,
          Arg[1].Cargo_over_water,
          Arg[1].Container_Enclosed,
          Arg[1].ParatrooperGroupSpawn,
          Arg[1].offload_cargo,
          Arg[1].all_cargo_survive_to_the_ground,
          Arg[1].all_cargo_gets_destroyed,
          Arg[1].destroy_cargo_dropped_without_parachute,
          Arg[1].Cargo_Country
        )
...
function Hercules_Cargo.birthEvent:onEvent(event)
  if event.id == 15 then
    ...
    if desc["displayName"] == "Hercules" then
      ...
      missionCommands.addCommandForGroup(grpTab["object"]["id_"], "MLRS", hercCargoMenu, function()
        Hercules_Cargo.setType(grpTab["key"], "MLRS", 1)
      end, nil)
      missionCommands.addCommandForGroup(grpTab["object"]["id_"], "Mortar", hercCargoMenu, function()
        Hercules_Cargo.setType(grpTab["key"], "2B11 mortar", 8)
      end, nil)
      missionCommands.addCommandForGroup(grpTab["object"]["id_"], "M-109", hercCargoMenu, function()
        Hercules_Cargo.setType(grpTab["key"], "M-109", 1)
      end, nil)
      missionCommands.addCommandForGroup(grpTab["object"]["id_"], "FOB Crate", hercCargoMenu, function()
        Hercules_Cargo.setType(grpTab["key"], "Container red 1", 1)
      end, nil)
    end
  end
end
