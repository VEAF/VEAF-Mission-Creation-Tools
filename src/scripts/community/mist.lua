...
do -- the main scope
          ...
          for cntry_id, cntry_data in pairs(coa_data.country) do
            ...
            if type(cntry_data) == "table" then --just making sure
              for obj_cat_name, obj_cat_data in pairs(cntry_data) do
                if
                  obj_cat_name == "helicopter"
                  or obj_cat_name == "ship"
                  or obj_cat_name == "plane"
                  or obj_cat_name == "vehicle"
                  or obj_cat_name == "static"
                then --should be an unncessary check
                  local category = obj_cat_name

                  if
                    (type(obj_cat_data) == "table")
                    and obj_cat_data.group
                    and (type(obj_cat_data.group) == "table")
                    and (#obj_cat_data.group > 0)
                        ...
                        for unit_num, unit_data in pairs(group_data.units) do
                          ...
                          if category == "static" then
                            ...
                            if group_data.linkOffset then
                              if
                                group_data.route
                                and group_data.route.points
                                and group_data.route.points[1]
                                and group_data.route.points[1].linkUnit
                              then
            ...
            if uRef then
              if zone.verticies then
                ...
                zone.offset = {
                  ...
                  heading = mist.getHeadingPoints(uRef.point, zone.point) + uRef.heading,
                }
              end
            end
          end
        ...
        for unitId, unitData in pairs(unitOneRef) do
          ...
          for index, data in pairs(mistAddedObjects) do
            ...
            if newTable.units[unitId].alt_type == "RADIO" then -- raw postition MSL was grabbed for group, but spawn is AGL, so re-offset it
              newTable.units[unitId].alt = (
                newTable.units[unitId].alt - land.getHeight({ x = newTable.units[unitId].x, y = newTable.units[unitId].y })
              )
            end
          end
        end
            ...
            if gData.type ~= "static" then
              ...
              if _g and _g:isExist() == true then
                ...
                if
                  _u
                  and (
                    dbTable.groupId ~= tonumber(_g:getID())
                    or _u:getCountry() ~= dbTable.countryId
                    or _u:getCoalition() ~= dbTable.coaltionId
                  )
                then
  ...
  function mist.dynAddStatic(n)
    ...
    if
      ...
      and newObj.y
      and newObj.type
      and type(newObj.x) == "number"
      and type(newObj.y) == "number"
      and type(newObj.type) == "string"
    then
      --log:warn(newObj)
  ...
  function mist.dynAdd(ng)
    ...
    for unitIndex, unitData in pairs(newGroup.units) do
      ...
      if newCat == "AIRPLANE" or newCat == "HELICOPTER" then
        if
          newGroup.units[unitIndex].alt_type and newGroup.units[unitIndex].alt_type ~= "BARO" or not newGroup.units[unitIndex].alt_type
        then
...
do
  ...
  function mist.getAttitude(unit)
    ...
    if unitpos then
      ...
      if mist.vec.mag(unitvel) ~= 0 then --must have non-zero velocity!
        ...
        Yaw = math.acos(
          mist.vec.dp({ x = 1, y = 0, z = 0 }, { x = AxialVel.x, y = 0, z = AxialVel.z })
        ...
        AoA = math.acos(
          mist.vec.dp({ x = 1, y = 0, z = 0 }, { x = AxialVel.x, y = AxialVel.y, z = 0 })
  ...
  function mist.getUnitsInPolygon(unit_names, polyZone, max_alt)
    ...
    for i = 1, #units do
      ...
      if
        lUnit:isExist() == true
        and ((lCat == 1 and lUnit:isActive()) or lCat ~= 1)
        and mist.pointInPolygon(lUnit:getPosition().p, polyZone, max_alt)
      then
      ...
      for zones_ind = 1, #zones do
        ...
        if unit_pos and ((lCat == 1 and lUnit:isActive() == true) or lCat ~= 1) then -- it is a unit and is active or it is not a unit
          if zones[zones_ind].verts then
            ...
            if
              zone_type == "cylinder"
              and (((unit_pos.x - zones[zones_ind].x) ^ 2 + (unit_pos.z - zones[zones_ind].z) ^ 2) ^ 0.5 <= zones[zones_ind].radius)
            then
              in_zone_units[#in_zone_units + 1] = lUnit
              break
            elseif
      ...
      for zone_units_ind = 1, #zone_units do
        ...
        if unit_pos and zone_unit_pos and ((lCat == 1 and lUnit:isActive()) or lCat ~= 1) then
          ...
          elseif
            zone_type == "sphere"
            and (
              ((unit_pos.x - zone_unit_pos.x) ^ 2 + (unit_pos.y - zone_unit_pos.y) ^ 2 + (unit_pos.z - zone_unit_pos.z) ^ 2) ^ 0.5 <= radius
            )
          then
            in_zone_units[#in_zone_units + 1] = lUnit
            break
  ...
  function mist.getPointAtDistanceOnPath(p, dist, r, rtn)
    ...
    for i = 1, #path do
      ...
      if l > dist then
        ...
        local x, y
        if r then
          x, y = land.getClosestPointOnRoads(
            rType,
            mist.utils.round((math.cos(dir) * diff) + path[i].x, 1),
...
do -- group functions scope
  ...
  function mist.groupTableCheck(groupData)
    ...
    for unitId, unitData in pairs(groupData.units) do
      if not unitData.x or not unitData.y or not unitData.type then
        return false
      end
    end
  ...
  function mist.getCurrentGroupData(gpName)
    ...
    if Group.getByName(gpName) and Group.getByName(gpName):isExist() == true then
      ...
      for unitNum, unitData in pairs(newGroup:getUnits()) do
        ...
        if
          mist.DBs.unitsByName[uName]
          and unitData:getTypeName() == mist.DBs.unitsByName[uName].type
          and mist.DBs.unitsByName[uName].unitId == tonumber(unitData:getID())
        then -- If old data matches most of new data
  ...
  function mist.getValidRandomPoint(vars) end
      ...
      if point then
        if newGroupData.category == "plane" or newGroupData.category == "helicopter" then
          if
            point.z
            and point.y > 0
            and point.y > land.getHeight({ newGroupData.units[unitNum].x, newGroupData.units[unitNum].y }) + 10
          then
            ...
            if newGroupData.category == "plane" then
              --log:info('setNewAlt')
              newGroupData.units[unitNum].alt = land.getHeight({ newGroupData.units[unitNum].x, newGroupData.units[unitNum].y })
                + math.random(300, 9000)
            else
              newGroupData.units[unitNum].alt = land.getHeight({ newGroupData.units[unitNum].x, newGroupData.units[unitNum].y })
                + math.random(200, 3000)
            end
          end
        end
      end
      ...
      for i = 1, #sorted do
        ...
        elseif type(val) == "table" then
          if tableshow_tbls[val] then
            ...
            --tbl_str[#tbl_str + 1] = tostring(val) .. ' '
            tbl_str[#tbl_str + 1] = mist.utils.tableShowSorted(
              val,
              { loc = loc .. '["' .. ind .. '"]', indent = indent .. "    ", tableshow_tbls = tableshow_tbls }
            )
            tbl_str[#tbl_str + 1] = ",\n"
          end
      ...
      for ind, val in pairs(tbl) do
        ...
        elseif type(val) == "table" then
          if tableshow_tbls[val] then
            ...
            tbl_str[#tbl_str + 1] = tostring(val) .. " "
            tbl_str[#tbl_str + 1] =
              mist.utils.tableShow(val, loc .. "[" .. mist.utils.basicSerialize(ind) .. "]", indent .. "    ", tableshow_tbls)
            tbl_str[#tbl_str + 1] = ",\n"
          end
        elseif type(val) == "function" then
          if debug and debug.getinfo then
...
do -- mist.debug scope
  ...
  function mist.debug.writeWeapons(unit) end
...
initial_number
    ...
    if
      stopflag == -1
      or (type(trigger.misc.getUserFlag(stopflag)) == "number" and trigger.misc.getUserFlag(stopflag) == 0)
...
stopFlag
  ...
  function mist.flagFunc.group_dead(vars)
    ...
    then
      if
        (Group.getByName(groupName) and Group.getByName(groupName):isExist() == false)
        or (Group.getByName(groupName) and #Group.getByName(groupName):getUnits() < 1)
        or not Group.getByName(groupName)
      then
        if trigger.misc.getUserFlag(flag) == 0 then
          trigger.action.setUserFlag(flag, true)
        end
          ...
          if caSlots == true and caMSGtoGroup == false then
            if msgTableText.RED then
              trigger.action.outTextForCoalition(
                coalition.side.RED,
                table.concat(msgTableText.RED.text),
                msgTableText.RED.displayTime,
                clearView
              )
            end
            if msgTableText.BLUE then
              trigger.action.outTextForCoalition(
                coalition.side.BLUE,
                table.concat(msgTableText.BLUE.text),
                msgTableText.BLUE.displayTime,
                clearView
              )
            end
          end
      ...
      for forIndex, forData in pairs(vars.msgFor) do
        for list, listData in pairs(forData) do
          for clientId, clientData in pairs(mist.DBs.humansById) do
            ...
            if
              (forIndex == "coa" and (listData == string.lower(clientData.coalition) or listData == "all"))
              or (forIndex == "countries" and string.lower(clientData.country) == listData)
              or (forIndex == "units" and string.lower(clientData.unitName) == listData)
            then --
...
vars.msgFor - scope
  ...
  function mist.msgLeadingLL(vars)
    ...
    local s =
      mist.getLeadingLLString({ units = units, heading = heading, radius = radius, headingDegrees = headingDegrees, acc = acc, DMS = DMS })
    local newText
...
vars.msgFor - scope
  ...
  function mist.msgLeadingBR(vars)
    ...
    local s = mist.getLeadingBRString({
      units = units,
      heading = heading,
      radius = radius,
    ...
    if unit:isExist() then
        ...
        if angles then
          ...
          trigger.action.outText(
            string.format("%-25s", "Heading: ")
              ...
              .. absVel
              .. " m/s\n"
              .. string.format("%-25s", "Absolute Acceleration: ")
              .. unitAcc
              .. " m/s^2\n"
      ...
      local function frameFinder(unit, prevVel, prevE, prevTime)
        if unit:isExist() then
          ...
          if
            prevVel and (prevVel.x ~= currVel.x or prevVel.y ~= currVel.y or prevVel.z ~= currVel.z)
            or (prevTime and (timer.getTime() - prevTime) > 0.25)
          then
...
do
  ...
  local mDefs = {
    coa = {
      ["red"] = { fillColor = { 0.8, 0, 0, 0.5 }, color = { 0.8, 0, 0, 0.5 }, lineType = 2, fontSize = 16 },
      ["blue"] = { fillColor = { 0, 0, 0.8, 0.5 }, color = { 0, 0, 0.8, 0.5 }, lineType = 2, fontSize = 16 },
      ["all"] = { fillColor = { 0.1, 0.1, 0.1, 0.5 }, color = { 0.9, 0.9, 0.9, 0.5 }, lineType = 2, fontSize = 16 },
      ["neutral"] = { fillColor = { 0.1, 0.1, 0.1, 0.5 }, color = { 0.2, 0.2, 0.2, 0.5 }, lineType = 2, fontSize = 16 },
    },
  }
    ...
    if world.event.S_EVENT_MARK_ADDED == e.id and e.idx then
      ...
      if not mist.DBs.markList[e.idx] then
        ...
        mist.DBs.markList[e.idx] =
          { time = e.time, pos = e.pos, groupId = e.groupId, mType = "panel", text = e.text, markId = e.idx, coalition = e.coalition }
        if e.unit then
          ...
          for list, listData in pairs(forData) do
            ...
            elseif forIndex == "countries" then
              for clienId, clientData in pairs(mist.DBs.humansById) do
                if
                  (string.lower(clientData.country) == listData) or (forIndex == "units" and string.lower(clientData.unitName) == listData)
                then
            ...
            elseif forIndex == "unittypes" then -- mark to group
              ...
              for clientId, clientData in pairs(mist.DBs.humansById) do
                for typeId, typeData in pairs(listData) do
                  ...
                  if
                    list == "all"
                    or clientData.coalition and type(clientData.coalition) == "string" and mist.stringMatch(clientData.coalition, list)
                  then
...
do -- mist.time scope
  ...
  function mist.getClockString(theTime, hour)
    ...
    if hour then
      if DHMS.h > 12 then
        DHMS.h = DHMS.h - 12
        return tostring(
          string.format("%02d", DHMS.h) .. ":" .. string.format("%02d", DHMS.m) .. ":" .. string.format("%02d", DHMS.s) .. " PM"
        )
      else
        return tostring(
          string.format("%02d", DHMS.h) .. ":" .. string.format("%02d", DHMS.m) .. ":" .. string.format("%02d", DHMS.s) .. " AM"
        )
      end
    else
  ...
  function mist.getDateString(rtnType, murica, oTime) -- returns date based on time
    local word =
      { "January", "Feburary", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" } -- 'etc
    local curTime = 0
    if oTime then
      curTime = oTime
    else
  ...
  function mist.ground.patrolRoute(vars)
    ...
    if routeProvided == false and #tempRoute > 0 then
      ...
      for i = 1, #tempRoute do
        ...
        if
          string.lower(tempRoute[i].action) == "on road"
          or string.lower(tempRoute[i].action) == "onroad"
          or string.lower(tempRoute[i].action) == "on_road"
        then
          tempForm = onRoadForm
        else
