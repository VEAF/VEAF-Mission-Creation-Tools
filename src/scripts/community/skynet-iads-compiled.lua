...
it, the reason is written at the call site.
  ...
  function SkynetIADSUtils.scheduleFunction(fn, args, startTime, repeatInterval, stopTime)
    ...
    assert(type(repeatInterval) == "number" or repeatInterval == nil, "scheduleFunction: argument 4 must be a number or nil")
...
do
  ...
  function SkynetIADSLogger:printEarlyWarningRadarStatus()
    ...
    self:printOutputToLog(
      "------------------------------------------ EW RADAR STATUS: " .. self.iads:getCoalitionString() .. " -------------------------------"
    )
    for i = 1, #ewRadars do
      ...
      self:printOutputToLog(
        "ACTIVE: "
          ...
          .. #detectedTargets
          .. " | DEFENDING HARM: "
          .. tostring(ewRadar:isDefendingHARM())
      )
      ...
      if numPowerSources > 0 then
        self:printOutputToLog(
          "POWER SOURCES : " .. numPowerSources .. " | DAMAGED:" .. numDamagedPowerSources .. " | INTACT: " .. intactPowerSources
        )
      else
      ...
      for j = 1, #detectedTargets do
        ...
        if firstRadar ~= nil and firstRadar:isExist() then
          local distance = SkynetIADSUtils.round(
            SkynetIADSUtils.metersToNM(ewRadar:getDistanceInMetersToContact(firstRadar:getDCSRepresentation(), contact:getPosition().p)),
            2
          )
  ...
  function SkynetIADSLogger:printSAMSiteStatus()
    ...
    self:printOutputToLog(
      "------------------------------------------ SAM STATUS: " .. self.iads:getCoalitionString() .. " -------------------------------"
    )
    for i = 1, #samSites do
      ...
      self:printOutputToLog(
        "ACTIVE: "
          .. tostring(isActive)
          .. " | AUTONOMOUS: "
          .. tostring(isAutonomous)
          .. " | IS ACTING AS EW: "
          .. tostring(samSite:getActAsEW())
          .. " | CAN ENGAGE AIR WEAPONS : "
          .. tostring(engageAirWeapons)
          .. " | CAN ENGAGE HARMS : "
          .. tostring(engageHARMS)
          .. " | HAS AMMO: "
          .. tostring(hasAmmo)
          .. " | DETECTED TARGETS: "
          .. #detectedTargets
          .. " | DEFENDING HARM: "
          .. tostring(samSite:isDefendingHARM())
          .. " | MISSILES IN FLIGHT: "
          .. tostring(samSite:getNumberOfMissilesInFlight())
      )
      ...
      for j = 1, #detectedTargets do
        ...
        if firstRadar ~= nil and firstRadar:isExist() then
          local distance = SkynetIADSUtils.round(
            SkynetIADSUtils.metersToNM(samSite:getDistanceInMetersToContact(firstRadar:getDCSRepresentation(), contact:getPosition().p)),
            2
          )
          self:printOutputToLog("CONTACT: " .. contact:getName() .. " | TYPE: " .. contact:getTypeName() .. " | DISTANCE NM: " .. distance)
        end
  ...
  function SkynetIADSLogger:printCommandCenterStatus()
    ...
    for i = 1, #commandCenters do
      ...
      if connectionNodeInfo.numSources > 0 then
        self:printOutputToLog(
          "CONNECTION NODES: "
            .. connectionNodeInfo.numSources
            .. " | DAMAGED: "
            .. connectionNodeInfo.numDamagedSources
            .. " | INTACT: "
            .. connectionNodeInfo.numIntactSources
        )
      else
        self:printOutputToLog("NO CONNECTION NODES SET")
      end
      if powerSourceInfo.numSources > 0 then
        self:printOutputToLog(
          "POWER SOURCES : "
            .. powerSourceInfo.numSources
            .. " | DAMAGED: "
            .. powerSourceInfo.numDamagedSources
            .. " | INTACT: "
            .. powerSourceInfo.numIntactSources
        )
      else
        self:printOutputToLog("NO POWER SOURCES SET")
      end
    ...
    if self:getDebugSettings().IADSStatus then
      ...
      self:printOutput(
        "COMMAND CENTERS: "
          .. numComCenters
          .. " | Destroyed: "
          .. numDestroyedComCenters
          .. " | NoPowr: "
          .. numComCentersNoPower
          .. " | NoCon: "
          .. numComCentersNoConnectionNode
      )
        ...
        for i = 1, #contacts do
          ...
          self:printOutput(
            "CONTACT: "
              ...
              .. contact:getTypeName()
              .. " | GS: "
              .. tostring(contact:getGroundSpeedInKnots())
              .. " | LAST SEEN: "
              .. contact:getAge()
          )
        end
...
do
  ...
  function SkynetIADSAbstractElement:weaponFired(event) end

  --placeholder method, can be implemented by subclasses
  function SkynetIADSAbstractElement:goDark() end

  --placeholder method, can be implemented by subclasses
  function SkynetIADSAbstractElement:goAutonomous() end

  --placeholder method, can be implemented by subclasses
  function SkynetIADSAbstractElement:setToCorrectAutonomousState() end

  --placeholder method, can be implemented by subclasses
  function SkynetIADSAbstractElement:informChildrenOfStateChange() end
end
do
  ...
  function SkynetIADSAbstractRadarElement:setToCorrectAutonomousState()
    ...
    for i = 1, #parents do
      ...
      if
        self:hasActiveConnectionNode()
        and self.iads:isCommandCenterUsable()
  ...
  function SkynetIADSAbstractRadarElement:setIgnoreHARMSWhilePointDefencesHaveAmmo(state)
    self.iads:printOutputToLog(
      "DEPRECATED: setIgnoreHARMSWhilePointDefencesHaveAmmo SAM Site will stay live automaticall as long as itself or it's point defences can defend against a HARM"
    )
    return self
  end
    ...
    for typeName, dataType in pairs(SkynetIADS.database) do
      ...
      if
        (
          ...
          and #self.launchers > 0
          and #self.searchRadars > 0
          and #self.trackingRadars > 0
  ...
  function SkynetIADSAbstractRadarElement:goLive()
    if (self.aiState == false and self:hasWorkingPowerSource() and self.harmSilenceID == nil) and (self:hasRemainingAmmo() == true) then
      if self:isDestroyed() == false then
  ...
  function SkynetIADSAbstractRadarElement:goDark()
    if
      (self:hasWorkingPowerSource() == false)
      or (self.aiState == true)
  ...
  function SkynetIADSAbstractRadarElement:goSilentToEvadeHARM(timeToImpact)
    ...
    if self.iads:getDebugSettings().harmDefence then
      self.iads:printOutputToLog(
        "HARM DEFENCE SHUTTING DOWN: " .. self:getDCSName() .. " | FOR: " .. self.harmShutdownTime .. " seconds | TTI: " .. timeToImpact
      )
    end
    self.harmSilenceID = SkynetIADSUtils.scheduleFunction(
      SkynetIADSAbstractRadarElement.finishHarmDefence,
      { self },
      timer.getTime() + self.harmShutdownTime,
      1
    )
    self:goDark()
  end
  ...
  function SkynetIADSAbstractRadarElement:getDetectedTargets()
    if
      (timer.getTime() - self.cachedTargetsCurrentAge > self.cachedTargetsMaxAge)
      or (timer.getTime() - self.goLiveTime < self.noCacheActiveForSecondsAfterGoLive)
    then
      self.cachedTargets = {}
  ...
  function SkynetIADSAbstractRadarElement:shallIgnoreHARMShutdown()
    ...
    return (
      (
        self:hasEnoughLaunchersToEngageMissiles(numOfHarms)
        and self:hasRemainingAmmoToEngageMissiles(numOfHarms)
        and self:getCanEngageHARM()
      ) or (self:pointDefencesHaveRemainingAmmo(numOfHarms) and self:pointDefencesHaveEnoughLaunchers(numOfHarms))
    )
  end
  ...
  function SkynetIADSAbstractRadarElement:informOfHARM(harmContact)
    ...
    for j = 1, #radars do
      ...
      if radar:isExist() then
        ...
        local harmToSAMHeading =
          SkynetIADSUtils.toDegree(SkynetIADSUtils.getHeadingPoints(harmContact:getPosition().p, radar:getPosition().p))
        ...
        if
          harmToSAMAspect < SkynetIADSAbstractRadarElement.HARM_TO_SAM_ASPECT
          and distanceNM < SkynetIADSAbstractRadarElement.HARM_LOOKAHEAD_NM
        then
...
do
  ...
  function SkynetIADSSAMSearchRadar:getDistance(target)
    return SkynetIADSUtils.get2DDist(target:getPosition().p, self:getDCSRepresentation():getPosition().p)
  end
...
do
  ...
  function SkynetIADSSamSite:informOfContact(contact)
    ...
    if
      ...
      and (contact:isIdentifiedAsHARM() == false or (contact:isIdentifiedAsHARM() == true and self:getCanEngageHARM() == true))
    then
      self:goLive()
...
do
  ...
  function SkynetIADSHARMDetection:evaluateContacts()
    ...
    for i = 1, #self.contacts do
      ...
      if
        #newRadarsToEvaluate > 0
        and contact:isIdentifiedAsHARM() == false
        and (groundSpeed > SkynetIADSHARMDetection.HARM_THRESHOLD_SPEED_KTS and #simpleAltitudeProfile <= 2)
      then
        ...
        if self:shallReactToHARM(detectionProbability) then
          ...
          if self.iads:getDebugSettings().harmDefence then
            self.iads:printOutputToLog(
              "HARM IDENTIFIED: " .. contact:getTypeName() .. " | DETECTION PROBABILITY WAS: " .. detectionProbability .. "%"
            )
          end
        else
          ...
          if self.iads:getDebugSettings().harmDefence then
            self.iads:printOutputToLog(
              "HARM NOT IDENTIFIED: " .. contact:getTypeName() .. " | DETECTION PROBABILITY WAS: " .. detectionProbability .. "%"
            )
          end
        end
      end
