...
function DcssbMatchManager:new(objectToCopy)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager:new()"))
  end
...
function DcssbMatchManager:setName(value)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager[]:setName(%s)", value or "NONE"))
  end
  self.name = value
  return self
end
...
function DcssbMatchManager:setMatchName(value)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager[%s]:setMatchName(%s)", self:getName(), value or "NONE"))
  end
...
function DcssbMatchManager:setCoalition(value)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager[%s]:setCoalition(%s)", self:getName(), value or "NONE"))
  end
...
function DcssbMatchManager:setTriggerZone(value)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager[%s]:setTriggerZone(%s)", self:getName(), value or "NONE"))
  end
  if value then
...
function DcssbMatchManager:setTimeout(value)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager[%s]:setTimeout(%s)", self:getName(), value or "NONE"))
  end
  self.timeout = value
  return self
end
...
function DcssbMatchManager:addPlayerByName(playerName)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager[%s]:addPlayerByName(%s)", self:getName(), playerName or "NONE"))
  end
  if playerName then
    if dcsbot then
      dcsbot.sendBotTable({
    ...
    if DcssbMatchManager.LOG then
      env.info(string.format("DcssbMatchManager[%s]: Player %s added to match %s", self:getName(), playerName, self:getMatchName()))
    end
  end
...
function DcssbMatchManager:addPlayerByUnit(unitName)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager[%s]:addPlayerByUnit(%s)", self:getName(), unitName or "NONE"))
  end
  if unitName then
...
function DcssbMatchManager:addUnitWaitingToPassInTriggerZoneToBeAdded(unitName)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager[%s]:addUnitWaitingToPassInTriggerZoneToBeAdded(%s)", self:getName(), unitName or "NONE"))
  end
  if unitName then
...
function DcssbMatchManager:onSchedule()
  if DcssbMatchManager.TRACE then
    env.info(string.format("DcssbMatchManager[%s]:onSchedule()", self:getName()))
  end
  ...
  if self:getTriggerZone() then
    for unitName, _ in pairs(self.unitsWaitingToPassInTriggerZoneToBeAdded) do
      if DcssbMatchManager.LOG then
        env.info(
          string.format(
            "DcssbMatchManager[%s]:onSchedule() - player in unit %s is waiting to pass in trigger zone",
            self:getName(),
            unitName
          )
        )
      ...
      if not unit then
        if DcssbMatchManager.LOG then
          env.info(string.format("DcssbMatchManager[%s]:onSchedule() - player in unit %s is not in game anymore", self:getName(), unitName))
        end
        ...
        if ((unitPoint.x - zonePoint.x) ^ 2 + (unitPoint.z - zonePoint.z) ^ 2) ^ 0.5 <= zoneRadius then
          if DcssbMatchManager.LOG then
            env.info(
              string.format("DcssbMatchManager[%s]:onSchedule() - player in unit %s is passing in trigger zone", self:getName(), unitName)
            )
          end
          ...
          if self:getTimeout() then
            if DcssbMatchManager.LOG then
              env.info(
                string.format(
                  "DcssbMatchManager[%s]:onSchedule() - player in unit %s will be added to the match in %s seconds",
                  self:getName(),
                  unitName,
                  self:getTimeout()
                )
            ...
            if DcssbMatchManager.LOG then
              env.info(
                string.format(
                  "DcssbMatchManager[%s]:onSchedule() - player in unit %s is being added to the match",
                  self:getName(),
                  unitName
                )
              )
...
function DcssbMatchManager.addMatchManager(name, matchName, coalition, triggerZone, timeout)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager.addMatchManager(%s)", name or "NONE"))
  end
...
function DcssbMatchManager.addMatchManagersForZones(timeout, exclusionList)
  if DcssbMatchManager.LOG then
    env.info(string.format("DcssbMatchManager.addMatchManagersForZones(%s)", timeout or "NONE"))
  end
  local matchManagers = {}
  for _, zone in pairs(env.mission.triggers.zones) do
    ...
    if DcssbMatchManager.LOG then
      env.info(string.format("DcssbMatchManager.addMatchManagersForZones() - testing zone %s", name or "NONE"))
    end
    local exclude = false
    ...
    if not exclude then
      if zone then
        if DcssbMatchManager.LOG then
          env.info(string.format("DcssbMatchManager.addMatchManagersForZones() - adding zone %s", name or "NONE"))
        end
      ...
      if DcssbMatchManager.LOG then
        env.info(string.format("DcssbMatchManager.addMatchManagersForZones() - zone %s is excluded", name))
      end
    end
  end
...
function DcssbMatchManager.eventHandler:onEvent(event)
  local function completeUnitFromName(unitName)
    if DcssbMatchManager.TRACE then
      env.info(string.format("DcssbMatchManager.completeUnitFromName(unitName=%s)", unitName or "NONE"))
    end
  ...
  if DcssbMatchManager.TRACE then
    env.info(string.format("DcssbMatchManager.onEvent() - eventId=%s", eventId or "NONE"))
  end
    ...
    for _, matchManager in pairs(DcssbMatchManager.matchManagers) do
      ...
      if _coalitionMatches then
        ...
        if matchManager:getTriggerZone() then
          if DcssbMatchManager.LOG then
            env.info(
              string.format("DcssbMatchManager.onEvent() - player %s is waiting to pass in trigger zone", _event.initiator.unitPilotName)
            )
          end
        ...
        if matchManager:getTimeout() then
          if DcssbMatchManager.LOG then
            env.info(
              string.format(
                "DcssbMatchManager.onEvent() - player %s will be added to the match in %s seconds",
                _event.initiator.unitPilotName,
                matchManager:getTimeout()
              )
            )
          end
          timer.scheduleFunction(
            DcssbMatchManager.addPlayerByNameForScheduler,
            { matchManager, _event.initiator.unitPilotName },
            timer.getTime() + matchManager:getTimeout()
          )
        else
          if DcssbMatchManager.LOG then
            env.info(string.format("DcssbMatchManager.onEvent() - player %s is being added to the match", _event.initiator.unitPilotName))
          end
...
function DcssbMatchManager.initialize()
  if DcssbMatchManager.LOG then
    env.info("DcssbMatchManager.initialize() version " .. DcssbMatchManager.Version)
  end
