...
function trainingSpawnZone.activateZone(zoneName)
  ...
  -- VMR-074: an unknown zone name -- or the right name in the wrong case, which is why the
  -- lookup lowercases -- yields nil here, and the next line indexed it straight away. A typo
  -- crashed the command instead of being refused.
  if not _zone then
    trainingSpawnZone.logError(string.format("unknown zone [%s]", trainingSpawnZone.p(zoneName)))
    return false
  end
