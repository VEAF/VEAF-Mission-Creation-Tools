...
function DcsDataExport._p(objKey, objValue, maxLevel, level, skip, serializeInLua)
  ...
  if type(objValue) == "table" then
    if (maxLevel and level >= maxLevel) or (skip and skip[objKey]) then
    ...
    if fullDcsUnit then
      ...
      if unit["attribute"] then
        ...
        for _, attr in pairs(unit["attribute"]) do
          if type(attr) == "string" then
            if attr:lower() == "ships" then
              u.naval = true
            end
            if attr:lower() == "air" then
              u.air = true
            end
            if attr:lower() == "infantry" then
              u.infantry = true
            end
            if attr:lower() == "vehicles" then
              u.vehicle = true
            end
          end
        end
      ...
      if u.category == "Cargo" then
        ...
        if
          ...
          and unit["maxMass"]
          and unit["minMass"]
          and type(unit["maxMass"]) == "number"
          and type(unit["minMass"]) == "number"
        then
