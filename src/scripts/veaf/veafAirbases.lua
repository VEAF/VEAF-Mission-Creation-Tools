...
function veafAirbases.getNearestAirbase(dcsUnit)
  ...
  if nearestList and #nearestList >= 1 then
    ...
    veaf.loggers
      .get(veafAirbases.Id)
      :trace(string.format("Nearest airbase for [ %s ]: [ %s ] at %dm", dcsUnit:getName(), veafAirbase:toString(), nearestList[1][2]))
...
function veafAirbase:create(dcsAirbase)
  ...
  veaf.loggers
    .get(veafAirbases.Id)
    :trace(string.format(">> Airbase - %s created with display name  %s", dcsAirbase:getName(), sDisplayName))
...
function veafAirbaseRunway:create(dcsAirbase, dcsRunway, iReportOrder)
  ...
  if
    _numbersOffest(iDcsNumber, iOppositeNumberFromHeading) <= iFlipThreshold
    or _numbersOffest(iDcsNumber, iNumberFromHeading) > iFlipThreshold
  then
