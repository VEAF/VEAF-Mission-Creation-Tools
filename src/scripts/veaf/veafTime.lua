...
function veafTime.getMissionAbsTime(dateTime)
  ...
  local iAbsTime = iSecond + (iMinute * _iSecondsInMinute) + (iHour * _iSecondsInHour) + (iTotalDays * _iSecondsInDay)
...
function veafTime.isAeronauticalNight(vec3, iAbsTime)
  ...
  local iSunriseSeconds = sunriseTime.hour * _iSecondsInHour
    + sunriseTime.min * _iSecondsInMinute
    + sunriseTime.sec
    - (30 * _iSecondsInMinute) -- sunrise - 30 min
...
function veafSunTimes:sunAngleTime(angle, Ftime, direction)
  ...
  local t = (-veafSunTimes.DMath.Msin(angle) - veafSunTimes.DMath.Msin(decl) * veafSunTimes.DMath.Msin(self.lat))
    / (veafSunTimes.DMath.Mcos(decl) * veafSunTimes.DMath.Mcos(self.lat))
