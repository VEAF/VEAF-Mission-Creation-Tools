...
class TestCtld(unittest.TestCase):
    ...
    test module ``test_lua_config_generator``, which covers the start-up call itself."""

    def test_ctld_enabled_emits_no_setting(self) -> None:
        lua = generate_config_lua(
            {"external_modules": {"ctld": {"enabled": True, "hoverPickup": True, "maximumCrates": 5}}}
        )
        self.assertNotIn("ctld.hoverPickup", lua)
        self.assertNotIn("ctld.maximumCrates", lua)

    def test_ctld_disabled_emits_no_setting(self) -> None:
