...
def _seq(values: list):  # type: ignore[no-untyped-def]
    """An InquirerPy-prompt stand-in whose .execute() returns each value in turn.

    Returning ``None`` simulates the user pressing Escape (InquirerPy's "skip").
    """
    it = iter(values)
    return type("P", (), {"execute": lambda self, _it=it: next(_it)})()


class TestRunWizardEscape:
    """Escape navigation: back one level, or quit at the top."""
    ...
    def test_escape_at_command_menu_quits(self) -> None:
        with patch.object(sys.stdout, "isatty", return_value=True):
            with patch("veaf_libs.preferences.get_last_command", return_value=""):
                with patch("veaf_libs.preferences.get_last_args", return_value={}):
                    with patch("veaf_libs.tui._mission_yaml_defaults", return_value={}):
                        with patch("InquirerPy.inquirer.select", return_value=_seq([None])):  # Escape
                            result = run_wizard()
        assert result == []

    def test_escape_on_first_prompt_returns_to_menu_then_quits(self) -> None:
        # select 'build', Escape on its first prompt → back to menu → Escape → quit
        with patch.object(sys.stdout, "isatty", return_value=True):
            with patch("veaf_libs.preferences.get_last_command", return_value="build"):
                with patch("veaf_libs.preferences.get_last_args", return_value={}):
                    with patch("veaf_libs.preferences.save_invocation"):
                        with patch("veaf_libs.tui._mission_yaml_defaults", return_value={}):
                            with patch("InquirerPy.inquirer.select", return_value=_seq(["build", None])):
                                with patch("InquirerPy.inquirer.text", return_value=_seq([None])):  # Escape
                                    result = run_wizard()
        assert result == []

    def test_escape_on_second_prompt_steps_back_then_completes(self) -> None:
        # build has two positional prompts; Escape on the 2nd returns to the 1st, which is
        # re-asked (then both answered) → a complete arg list is produced.
        with patch.object(sys.stdout, "isatty", return_value=True):
            with patch("veaf_libs.preferences.get_last_command", return_value="build"):
                with patch("veaf_libs.preferences.get_last_args", return_value={}):
                    with patch("veaf_libs.preferences.save_invocation"):
                        with patch("veaf_libs.tui._mission_yaml_defaults", return_value={}):
                            with patch("InquirerPy.inquirer.select", return_value=_seq(["build"])):
                                with patch(
                                    "InquirerPy.inquirer.text",
                                    return_value=_seq(["mission.miz", None, "mission.miz", "."]),
                                ):
                                    result = run_wizard()
        assert result == ["build", "mission.miz", "."]

    def test_escape_on_first_prompt_of_preselected_command_quits(self) -> None:
        # bridge-preselected command: no menu to fall back to → Escape on the first
        # (only) prompt quits.
        with patch.object(sys.stdout, "isatty", return_value=True):
            with patch("veaf_libs.preferences.get_last_command", return_value=""):
                with patch("veaf_libs.preferences.get_last_args", return_value={}):
                    with patch("veaf_libs.tui._mission_yaml_defaults", return_value={}):
                        with patch("InquirerPy.inquirer.select", return_value=_seq([None])):  # template Escape
                            result = run_wizard(preselected="prepare", provided={"mission_folder": "c:/tmp"})
        assert result == []
