...
| Item | What it is |
|------|------------|
| [`Convert-FootholdBatch.ps1`](Convert-FootholdBatch.ps1) | Adopts every Lekaa Foothold release archive of a folder in one pass, picking the right conversion profile per mission, and optionally validates and builds them all — flagging a built `.miz` whose name no longer matches `mission.yaml`. See [FOOTHOLD](../doc/mission-maker/FOOTHOLD.md). |
